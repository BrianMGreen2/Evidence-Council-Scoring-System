...
INTERACTIVE = True   # overridden at __main__ if "non-interactive" arg is passed
...
def run_demo():
    ...
    # If running in Docker with a mounted output volume, copy the JSONL there too
    output_dir = Path("/demo/output")
    if output_dir.exists() and output_dir.is_dir():
        import shutil
        dest = output_dir / DEMO_KL_PATH.name
        shutil.copy(DEMO_KL_PATH, dest)
        print(c(f"  Knowledge layer also saved to: {dest}", DIM))
...
if __name__ == "__main__":
    # Pass "non-interactive" as first argument to auto-advance between scenarios.
    # Used by: docker run -it --rm evidence-council-demo non-interactive
    INTERACTIVE = not (len(sys.argv) > 1 and sys.argv[1] == "non-interactive")
    run_demo()
