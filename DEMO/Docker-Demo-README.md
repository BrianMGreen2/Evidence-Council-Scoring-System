...
demo/
├── README.md                              ← this file
├── Dockerfile                             ← builds a self-contained demo image
...
## Running with Docker (recommended for live demos)

Docker Desktop is the recommended path for conference presentations. The image is fully self-contained — no Python installation, no wifi, no "works on my machine" variables on the day.

**One-time setup (do this before the conference):**
...
The build pulls `python:3.12-slim` and installs NumPy. Total image size is under 200 MB. Once built, it runs entirely offline.

**Interactive mode** (pauses between scenarios — use this for live presentations):

```bash
docker run -it --rm evidence-council-demo
```

Press `Enter` to advance through each scenario at your own pace. The `-it` flag is required — it allocates a TTY so ANSI colors and the interactive prompt render correctly in Docker Desktop's terminal.

**Non-interactive mode** (auto-advances with a 1.5s pause — use this for recordings):

```bash
docker run -it --rm evidence-council-demo non-interactive
```

**Save the knowledge layer JSONL to your host machine** after the run:

```bash
mkdir -p output
docker run -it --rm -v "$(pwd)/output":/demo/output evidence-council-demo
# governance_knowledge_layer_demo.jsonl will appear in ./output/
```

On Windows with Docker Desktop, replace `$(pwd)` with the full path:

```powershell
docker run -it --rm -v "C:\path\to\demo\output:/demo/output" evidence-council-demo
```

**Verify the image before the conference:**

```bash
# Confirm it builds and runs to completion without interaction
docker run -it --rm evidence-council-demo non-interactive | tail -5
# Expected last line: "Demo complete. All 5 governance scenarios demonstrated."
```

**Rebuilding after changes:** if you modify `demo_evidence_council.py`, rebuild with:

```bash
docker build --no-cache -t evidence-council-demo .
```
## NOTE WHAT WAS ADDED FOR DOCKER DEMO
